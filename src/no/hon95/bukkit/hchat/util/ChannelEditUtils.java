	...
	public static void edit(HChatPlugin plugin, CommandSender sender, Channel channel, String strAction, String strKey, String value) throws EditException {
		...
		if (key == Key.OWNER || key == Key.MEMBERS || key == Key.BANNED_MEMBERS) {
			...
			if (PlayerIdUtils.isUuidSupported()) {
				uuid = PlayerIdUtils.getPlayerUuid(lowerValue, true);
			} else {
				sender.sendMessage("§7The server is outdated or in offline mode, so the target player needs to be online. "
						+ "The local player UUID will be used, and be warned, it might change.");
				uuid = PlayerIdUtils.getLocalPlayerUuid(lowerValue);
			}
			if (uuid != null)
				id = uuid.toString();
			else
